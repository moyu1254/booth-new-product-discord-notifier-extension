...
BOOTH の指定タグに新商品が追加されたとき、拡張機能内の一覧と必要に応じて Discord Webhook へ通知するブラウザ拡張です。  
...
Discord 通知を有効にしている場合は、同じ商品を Discord Webhook にも送信します。成人向け商品は Discord 側でもタイトル、区分欄、色で判別しやすくしています。

## 導入方法

### GitHub Releases から導入

1. GitHub Releases から利用するブラウザ向けの ZIP をダウンロードして展開します
...
4. `Load Temporary Add-on...` から展開したフォルダ内の `manifest.json` を選択します
...
Firefox 版は別の manifest を使ってビルドします。
...
タグを push すると、`Package Extension` workflow が GitHub Release を作成し、`dist/packages/*.zip` を添付します。
...
- Discord Webhook URL はブラウザ同期ストレージではなく、端末ローカルの拡張ストレージに保存します
